...
bool sprite_manager::load_pak(std::string_view name, std::string_view path) {
    ...
    spdlog::info("Loaded PAK file '{}': {} ({} sprites)",
                 name, full_path.string(), pak_files_[name_str]->sprite_count());
    return true;
}
...
void sprite_manager::unload_pak(std::string_view name) {
    ...
    // Remove from LRU
    lru_order_.erase(
        std::remove_if(lru_order_.begin(), lru_order_.end(),
            [&name_str](const sprite_key& key) {
                return key.pak_name == name_str;
            }),
        lru_order_.end()
    );
...
void sprite_manager::preload_sprites(std::string_view pak_name, uint32_t start, uint32_t count) {
    ...
    spdlog::trace("Preloaded {} sprites from {}[{}..{}]",
                  loaded, pak_name, start, end - 1);
}
...
void sprite_manager::update_memory(float delta_time) {
    ...
    if (evicted > 0) {
        spdlog::trace("Memory eviction: unloaded {} sprites, freed ~{} KB",
            evicted, memory_freed / 1024);
    }
}
...
void sprite_manager::preload_metadata_range(std::string_view pak_name, uint32_t start, uint32_t count) {
    ...
    spdlog::trace("Preloaded metadata for {} sprites from {}[{}..{}]",
                  loaded, pak_name, start, end - 1);
}
