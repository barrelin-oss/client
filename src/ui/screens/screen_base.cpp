...
void screen_base::draw_sprite(
    renderer& rend, sprite_manager& sprites, uint16_t sprite_id, int32_t x, int32_t y, uint32_t frame)
{
...
void screen_base::draw_sprite_alpha(
...
void screen_base::draw_sprite_no_color_key(
    renderer& rend, sprite_manager& sprites, uint16_t sprite_id, int32_t x, int32_t y, uint32_t frame)
{
    const sprite* spr = sprites.get_sprite_by_id(sprite_id);
    if (spr)
...
void screen_base::draw_sprite_alpha_no_color_key(
    renderer& rend, sprite_manager& sprites, uint16_t sprite_id, int32_t x, int32_t y, uint32_t frame, float alpha)
{
    const sprite* spr = sprites.get_sprite_by_id(sprite_id);
    if (spr)
...
void screen_base::update_screen_offset(uint32_t window_width, uint32_t window_height)
{
    ...
    if (screen_offset_x_ < 0)
        screen_offset_x_ = 0;
    if (screen_offset_y_ < 0)
        screen_offset_y_ = 0;
}
