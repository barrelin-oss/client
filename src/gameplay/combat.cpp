...
bool combat_system::can_attack(entity_id attacker, entity_id target) const {
    ...
    if (att->should_remove() || tgt->should_remove()) return false;

    // Check if attacker can fight
    if (!att->has_combat()) return false;
    if (att->combat().attack_cooldown > 0) return false;
    if (att->combat().paralyzed || att->combat().frozen) return false;

    // Check if target is valid
    if (!tgt->has_stats()) return false;
    if (tgt->stats().hp <= 0) return false;
...
void combat_system::start_attack(entity_id attacker, entity_id target, attack_type type) {
    if (!can_attack(attacker, target)) return;
...
attack_type combat_system::get_attack_type(uint16_t weapon_type, uint8_t skill_mastery, bool use_super) const {
    ...
    if (use_super && skill_mastery >= 100) {
        ...
        switch (skill) {
            case weapon_skill::sword:    return attack_type::super_1;
            case weapon_skill::axe:      return attack_type::super_2;
            case weapon_skill::hammer:   return attack_type::super_3;
            case weapon_skill::two_hand: return attack_type::super_4;
            case weapon_skill::spear:    return attack_type::super_5;
            case weapon_skill::archery:  return attack_type::super_6;
            case weapon_skill::staff:    return attack_type::super_7;
            case weapon_skill::fist:     return attack_type::super_8;
            default: break;
        }
    }
...
bool combat_system::can_use_super_attack(entity_id attacker, attack_type super_type) const {
    if (!entities_ || !skills_) return false;
    if (super_type < attack_type::super_1 || super_type > attack_type::super_8) return false;
    ...
    switch (super_type) {
        case attack_type::super_1: skill = weapon_skill::sword; break;
        case attack_type::super_2: skill = weapon_skill::axe; break;
        case attack_type::super_3: skill = weapon_skill::hammer; break;
        case attack_type::super_4: skill = weapon_skill::two_hand; break;
        case attack_type::super_5: skill = weapon_skill::spear; break;
        case attack_type::super_6: skill = weapon_skill::archery; break;
        case attack_type::super_7: skill = weapon_skill::staff; break;
        case attack_type::super_8: skill = weapon_skill::fist; break;
...
bool combat_system::check_level_up(entity_id entity) {
    ...
    class entity* ent = entities_->get_entity(entity);
    if (!ent || !ent->has_stats()) return false;
