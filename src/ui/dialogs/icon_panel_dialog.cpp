...
icon_panel_dialog::icon_panel_dialog() : dialog(dialog_type::icon_panel)
{
...
void icon_panel_dialog::update(float delta_time, const input& inp)
{
    ...
    mouse_in_info_area_ = (mx >= info_area_x && mx < info_area_x + info_area_width && my >= panel_y + info_area_y &&
                           my < panel_y + info_area_y + info_area_height);
}
...
void icon_panel_dialog::render_modern(renderer& rend)
{
    ...
    rend.draw_rect(0, panel_y, static_cast<int32_t>(screen_width_), panel_height, sf::Color(20, 20, 30, 220), true);

    // Draw subtle top border
    rend.draw_rect(0, panel_y, static_cast<int32_t>(screen_width_), 1, sf::Color(60, 60, 80), true);
...
void icon_panel_dialog::render_modern_gauge_bars(renderer& rend)
{
    ...
    if (hp_fill_width > 0)
    {
        ...
        // Highlight at top
        sf::Color hp_highlight(static_cast<uint8_t>(std::min(255, hp_fill.r + 50)),
...
void icon_panel_dialog::render_modern_action_buttons(renderer& rend)
{
    ...
    static const char* button_tooltips[] = {
        "Character (F5)", "Inventory (F6)", "Magic (F7)", "Skills (F8)", "History (F9)", "System (F12)"};
    ...
    for (int32_t i = 0; i < button_count; ++i)
    {
        ...
        // Button background
        sf::Color bg_color = (hovered_button_ == i) ? sf::Color(70, 70, 90, 220) : sf::Color(40, 40, 55, 200);
        ...
        // Button label
        sf::Color text_color = (hovered_button_ == i) ? sf::Color::White : sf::Color(180, 180, 200);
        rend.draw_text(button_labels[i], btn_x + button_width / 2 - 4, btn_y + button_height / 2 - 6, text_color, 12);
        ...
        if (hovered_button_ == i)
        {
            ...
            rend.draw_rect(tooltip_x, tooltip_y, tooltip_width, 18, sf::Color(30, 30, 40, 240), true);
            rend.draw_rect(tooltip_x, tooltip_y, tooltip_width, 18, sf::Color(80, 80, 100), false);
            rend.draw_text(button_tooltips[i], tooltip_x + 4, tooltip_y + 2, sf::Color(200, 200, 120), 11);
        }
    }
}
...
void icon_panel_dialog::render_modern_combat_indicator(renderer& rend)
{
    ...
    sf::Color indicator_color = safe_attack_mode_ ? sf::Color(100, 200, 100) : // Green for safe mode
...
void icon_panel_dialog::render_classic_background(renderer& rend)
{
    ...
    const sprite* icon_spr =
        sprites_ ? sprites_->get_sprite(classic_sprites::pak_name, classic_sprites::sprite_index) : nullptr;
    ...
    else
    {
        ...
        rend.draw_rect(0, panel_y, static_cast<int32_t>(screen_width_), panel_height, sf::Color(32, 32, 48, 255), true);

        // Classic border styling
        rend.draw_line(0, panel_y, static_cast<int32_t>(screen_width_), panel_y, sf::Color(100, 100, 140));
        rend.draw_line(0,
                       panel_y + panel_height - 1,
                       static_cast<int32_t>(screen_width_),
                       panel_y + panel_height - 1,
                       sf::Color(20, 20, 30));
    }
}
...
void icon_panel_dialog::render_classic_gauge_bars(renderer& rend)
{
    ...
    if (icon_spr && icon_spr->frame_count() > classic_sprites::hp_mp_bar_fill && hp_fill_width > 0)
    {
        ...
        icon_spr->draw_width(
            rend.window(), classic_layout::hp_bar_x, hp_y, classic_sprites::hp_mp_bar_fill, hp_fill_width);
    ...
    {
        icon_spr->draw_width(
            rend.window(), classic_layout::hp_bar_x, mp_y, classic_sprites::hp_mp_bar_fill, mp_fill_width);
    }
    else
    {
        // Fallback: programmatic rendering
    ...
    {
        icon_spr->draw_width(
            rend.window(), classic_layout::sp_bar_x, sp_y, classic_sprites::sp_bar_fill, sp_fill_width);
    }
    else
    {
        // Fallback: programmatic rendering
...
void icon_panel_dialog::render_classic_action_buttons(renderer& rend)
{
    ...
    static const int32_t button_x_positions[] = {classic_layout::button_character_x,
    ...
    for (int32_t i = 0; i < button_count; ++i)
    {
        ...
        if (icon_spr && icon_spr->frame_count() > button_frames[i])
        {
            ...
            {
                icon_spr->draw_alpha(rend.window(), btn_x, btn_y, classic_sprites::panel_hover_highlight, 0.5f);
            }
        }
        else
        {
            ...
            if (!hovered)
            {
                ...
                rend.draw_line(
                    btn_x, btn_y, btn_x, btn_y + classic_layout::button_height - 1, sf::Color(100, 100, 130));
                rend.draw_line(btn_x + classic_layout::button_width - 1,
                               btn_y,
                               btn_x + classic_layout::button_width - 1,
                               btn_y + classic_layout::button_height - 1,
                               sf::Color(30, 30, 40));
                rend.draw_line(btn_x,
                               btn_y + classic_layout::button_height - 1,
                               btn_x + classic_layout::button_width - 1,
                               btn_y + classic_layout::button_height - 1,
                               sf::Color(30, 30, 40));
    ...
    if (combat_mode_)
    {
        ...
        uint32_t combat_frame = safe_attack_mode_ ? classic_sprites::combat_safe_mode : classic_sprites::combat_pk_mode;
        ...
        else
        {
            ...
            sf::Color indicator_color = safe_attack_mode_ ? sf::Color(100, 200, 100) : sf::Color(200, 100, 100);
...
int32_t icon_panel_dialog::get_hovered_button(int32_t mouse_x, int32_t mouse_y) const
{
    ...
    if (ui_style_ == ui_style::classic)
    {
        ...
        for (int32_t i = 0; i < button_count; ++i)
        {
            ...
            if (mouse_x >= btn_x && mouse_x < btn_x + classic_layout::button_width && mouse_y >= btn_y &&
                mouse_y < btn_y + classic_layout::button_height)
            {
                return i;
            }
        }
    ...
    else
    {
        ...
        for (int32_t i = 0; i < button_count; ++i)
        {
            ...
            if (mouse_x >= btn_x && mouse_x < btn_x + button_width && mouse_y >= btn_y &&
...
bool icon_panel_dialog::handle_mouse_down(int32_t x, int32_t y, sf::Mouse::Button btn)
{
    ...
    if (x >= combat_screen_x && x < combat_screen_x + 36 && y >= combat_screen_y && y < combat_screen_y + 36)
    {
        if (on_button_sound_)
            on_button_sound_();
