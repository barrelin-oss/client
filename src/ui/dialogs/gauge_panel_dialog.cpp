...
gauge_panel_dialog::gauge_panel_dialog()
    : dialog(dialog_type::none) {  // Custom HUD element
...
void gauge_panel_dialog::render(renderer& rend) {
    ...
    render_bar(rend, x, y, bar_width, bar_height, hp_display_,
               sf::Color(200, 50, 50), sf::Color(60, 20, 20), "HP", hp_text);

    y += bar_height + bar_spacing;

    // MP bar
    std::string mp_text = show_values_ ? std::to_string(current_mp_) + "/" + std::to_string(max_mp_) : "";
    render_bar(rend, x, y, bar_width, bar_height, mp_display_,
               sf::Color(50, 100, 200), sf::Color(20, 40, 80), "MP", mp_text);

    y += bar_height + bar_spacing;

    // SP bar
    std::string sp_text = show_values_ ? std::to_string(current_sp_) + "/" + std::to_string(max_sp_) : "";
    render_bar(rend, x, y, bar_width, bar_height, sp_display_,
               sf::Color(200, 180, 50), sf::Color(80, 70, 20), "SP", sp_text);

    y += bar_height + bar_spacing;

    // EXP bar
    std::string exp_percent = show_values_ ? std::to_string(static_cast<int>(exp_display_ * 100)) + "%" : "";
    render_bar(rend, x, y, bar_width, bar_height - 2, exp_display_,
               sf::Color(50, 200, 100), sf::Color(20, 70, 40), "EXP", exp_percent);
}

void gauge_panel_dialog::render_bar(renderer& rend, int32_t x, int32_t y, int32_t width, int32_t height,
    ...
    if (fill_width > 0) {
        ...
        sf::Color highlight(
            static_cast<uint8_t>(std::min(255, fill_color.r + 40)),
