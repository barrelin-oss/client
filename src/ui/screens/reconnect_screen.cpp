...
bool reconnect_screen::update(float delta_time, const input& inp)
{
    ...
    if (inp.is_mouse_pressed(sf::Mouse::Button::Left) && screen_width_ > 0)
    {
        ...
        if (mouse_x_ >= btn_x && mouse_x_ <= btn_x + button_width_ && mouse_y_ >= exit_btn_y &&
            mouse_y_ <= exit_btn_y + button_height_)
        {
            play_button_sound();
            if (on_exit_game_)
            {
...
void reconnect_screen::render(renderer& rend, sprite_manager& sprites)
{
    ...
    // Title - "HELBREATH" above, "XTREME" below with fire effect
    {
        ...
        rend.draw_text_outlined("HELBREATH",
                                hb_x,
                                hb_y,
        ...
        // Warm glow halo
        {
            ...
            for (const auto& g : rings)
            {
                ...
                rend.draw_text_outlined("XTREME",
                                        xt_x,
                                        xt_y,
        ...
        for (const auto& e : embers)
        {
            ...
            float ex = static_cast<float>(xt_x) + e.x * xt_w + 5.0f * std::sin(elapsed_time_ * 3.0f + e.phase);
            ...
            if (esz < 1)
                esz = 1;
            rend.draw_rect(static_cast<int32_t>(ex),
        ...
        rend.draw_text_outlined("XTREME", xt_x, xt_y, sf::Color(255, 230, 80), sf::Color(160, 30, 0), xt_size, 2.5f);
    ...
    // Exit button
    int32_t exit_btn_y = btn_y + button_height_ + 12;
    bool exit_hovered = mouse_x_ >= btn_x && mouse_x_ <= btn_x + button_width_ && mouse_y_ >= exit_btn_y &&
                        mouse_y_ <= exit_btn_y + button_height_;
    ...
    if (!status_text_.empty())
    {
        ...
        rend.draw_text_outlined(
            status_text_, status_x, status_y, sf::Color(180, 180, 180), sf::Color::Black, status_text_size, 1.0f);
    }
}
