...
character_create_dialog::character_create_dialog()
    : dialog(dialog_type::character_create) {
    set_title("Create Character");
    set_bounds({
        static_cast<int32_t>(screen_width) / 2 - 250,
        static_cast<int32_t>(screen_height) / 2 - 200,
        500, 400
    });
    set_closeable(true);
    set_modal(true);
...
void character_create_dialog::create_ui() {
    ...
    warrior_btn->set_on_click([this]() {
        char_data_.warrior = true;
    });
    ...
    mage_btn->set_on_click([this]() {
        char_data_.warrior = false;
    });
    ...
    hair_prev->set_text("<");
    hair_prev->set_on_click([this]() {
        char_data_.hair_style = (char_data_.hair_style + 7) % 8;
    });
    ...
    hair_next->set_text(">");
    hair_next->set_on_click([this]() {
        char_data_.hair_style = (char_data_.hair_style + 1) % 8;
    });
...
void character_create_dialog::render_character_preview(renderer& rend) {
    ...
    std::string hair_text = "Hair: " + std::to_string(char_data_.hair_style + 1) +
                           "/" + std::to_string(char_data_.hair_color + 1);
...
void character_create_dialog::update_stat_display() {
    if (str_value_label_) str_value_label_->set_text(std::to_string(char_data_.strength));
    if (vit_value_label_) vit_value_label_->set_text(std::to_string(char_data_.vitality));
    if (dex_value_label_) dex_value_label_->set_text(std::to_string(char_data_.dexterity));
    if (int_value_label_) int_value_label_->set_text(std::to_string(char_data_.intelligence));
    if (mag_value_label_) mag_value_label_->set_text(std::to_string(char_data_.magic));
    if (cha_value_label_) cha_value_label_->set_text(std::to_string(char_data_.charisma));
}
...
uint16_t character_create_dialog::get_stat(int index) const {
    switch (index) {
        case 0: return char_data_.strength;
        case 1: return char_data_.vitality;
        case 2: return char_data_.dexterity;
        case 3: return char_data_.intelligence;
        case 4: return char_data_.magic;
        case 5: return char_data_.charisma;
        default: return 10;
    }
}
...
void character_create_dialog::set_stat(int index, uint16_t value) {
    switch (index) {
        case 0: char_data_.strength = value; break;
        case 1: char_data_.vitality = value; break;
        case 2: char_data_.dexterity = value; break;
        case 3: char_data_.intelligence = value; break;
        case 4: char_data_.magic = value; break;
        case 5: char_data_.charisma = value; break;
    }
}

uint16_t character_create_dialog::get_stat_points_used() const {
    return (char_data_.strength - 10) + (char_data_.vitality - 10) +
           (char_data_.dexterity - 10) + (char_data_.intelligence - 10) +
           (char_data_.magic - 10) + (char_data_.charisma - 10);
}
