...
npc_dialog::npc_dialog()
    : dialog(dialog_type::npc_dialog) {
    set_title("Dialog");
    set_bounds({
        static_cast<int32_t>(screen_width) / 2 - 175,
        static_cast<int32_t>(screen_height) / 2 - 120,
        350,
        240
    });
    set_draggable(true);
...
void npc_dialog::render(renderer& rend) {
    ...
    for (size_t i = 0; i < options_.size(); ++i) {
        ...
        // Option bullet
        sf::Color text_color = opt.enabled
            ? (hovered ? sf::Color::Yellow : sf::Color::White)
