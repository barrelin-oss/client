...
bool ui_element::contains_point_absolute(int32_t screen_x, int32_t screen_y) const
{
    ...
    return screen_x >= abs_x && screen_x < abs_x + bounds_.width && screen_y >= abs_y &&
           screen_y < abs_y + bounds_.height;
}
...
void ui_text_input::render(renderer& rend)
{
    ...
    rend.draw_rect(
        abs_x, abs_y, bounds_.width, bounds_.height, focused_ ? sf::Color(120, 120, 160) : border_color_, false);
...
void ui_scrollbar::render(renderer& rend)
{
    ...
    if (vertical_)
    {
        ...
        rend.draw_rect(abs_x,
                       thumb_pos,
                       bounds_.width,
                       thumb_size,
                       dragging_ || hovered_ ? thumb_hover_color_ : thumb_color_,
                       true);
    }
    else
    {
...
bool ui_dropdown::point_in_header(int32_t x, int32_t y) const
{
    ...
    return x >= abs_x && x < abs_x + bounds_.width && y >= abs_y && y < abs_y + bounds_.height;
}
...
bool ui_dropdown::point_in_list(int32_t x, int32_t y) const
{
    ...
    return x >= abs_x && x < abs_x + bounds_.width && y >= list_y && y < list_y + list_height;
}

int32_t ui_dropdown::item_at_position(int32_t x, int32_t y) const
{
    if (!point_in_list(x, y))
        return -1;
...
bool ui_dropdown::handle_mouse_down(int32_t x, int32_t y, sf::Mouse::Button btn)
{
    ...
    if (expanded_)
    {
        ...
        if (x >= abs_x && x < abs_x + bounds_.width && y >= list_y && y < list_y + list_height)
        {
...
{
    if (!visible_ || !enabled_ || !expanded_)
        return false;
...
ui_rect ui_dropdown::expanded_bounds() const
{
    ...
    return ui_rect{bounds_.x, bounds_.y, bounds_.width, bounds_.height + list_height};
}
