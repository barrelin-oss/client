...
uint32_t packet::message_id() const
{
    ...
    return static_cast<uint32_t>(data_[3]) | (static_cast<uint32_t>(data_[4]) << 8) |
           (static_cast<uint32_t>(data_[5]) << 16) | (static_cast<uint32_t>(data_[6]) << 24);
}
...
std::span<const uint8_t> packet::payload() const
{
    ...
    return std::span<const uint8_t>(data_.data() + packet_header_size, data_.size() - packet_header_size);
}
...
std::span<uint8_t> packet::payload_mutable()
{
    ...
    return std::span<uint8_t>(data_.data() + packet_header_size, data_.size() - packet_header_size);
}
...
packet_reader::packet_reader(const packet& pkt) : data_(pkt.data()), size_(pkt.size()), pos_(packet_header_size) {}

packet_reader::packet_reader(const uint8_t* data, size_t size) : data_(data), size_(size), pos_(packet_header_size) {}
