...
managed_dialog::managed_dialog(dialog_definition def)
    ...
    set_bounds({definition_.bounds.x, definition_.bounds.y,
                definition_.bounds.width, definition_.bounds.height});
...
void managed_dialog::update(float delta_time, const input& inp) {
    ...
    if (definition_.closeable && definition_.has_title_bar) {
        ...
        close_button_hovered_ = (mx >= close_x && mx < close_x + 16 &&
                                  my >= close_y && my < close_y + 16);
    }
...
void managed_dialog::render(renderer& rend) {
    ...
    for (const auto& elem : definition_.elements) {
        ...
        if (it == element_states_.end()) continue;
...
bool managed_dialog::handle_mouse_down(int32_t x, int32_t y, sf::Mouse::Button btn) {
    ...
    if (definition_.draggable && definition_.has_title_bar) {
        if (y >= bounds_.y && y < bounds_.y + title_bar_height &&
            x >= bounds_.x && x < bounds_.x + bounds_.width) {
            is_dragging_ = true;
...
bool managed_dialog::handle_text_input(char32_t unicode) {
    ...
    if (focused_element_id_.empty()) return false;

    const element_def* elem = find_element_def(focused_element_id_);
    if (!elem || elem->type != element_type::text_input) return false;
    ...
    // Check max length
    if (static_cast<int32_t>(state.text.length()) >= elem->max_chars) return false;
...
ui_bounds managed_dialog::get_absolute_bounds(const element_def& elem) const {
    ...
    return {
        bounds_.x + elem.bounds.x,
        bounds_.y + title_offset + elem.bounds.y,
        elem.bounds.width,
        elem.bounds.height
    };
}
...
std::pair<int32_t, int32_t> managed_dialog::grid_cell_at_point(const element_def& elem,
