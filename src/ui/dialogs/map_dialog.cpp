...
void map_dialog::render_map_area(renderer& rend, int32_t x, int32_t y, int32_t width, int32_t height) {
    ...
    for (const auto& marker : markers_) {
        ...
        switch (marker.type) {
            case 0: should_show = true; break;  // Player always shown
            case 1: should_show = show_party_; break;
            case 2: should_show = show_enemies_; break;
            case 3: should_show = show_npcs_; break;
            case 4: should_show = show_teleports_; break;
        }
        ...
        if (mx >= x && mx < x + width && my >= y && my < y + height) {
            ...
            if (marker.type <= 1 && !marker.name.empty()) {
                rend.draw_text(marker.name, mx - static_cast<int32_t>(marker.name.length() * 2),
                              my - 12, marker.color, 9);
            }
        }
    }
...
std::pair<int32_t, int32_t> map_dialog::world_to_map(int32_t world_x, int32_t world_y,
                                                      int32_t area_x, int32_t area_y,
                                                      int32_t area_w, int32_t area_h) const {
...
std::optional<std::pair<int32_t, int32_t>> map_dialog::map_to_world(int32_t mx, int32_t my,
                                                                     int32_t area_x, int32_t area_y,
                                                                     int32_t area_w, int32_t area_h) const {
