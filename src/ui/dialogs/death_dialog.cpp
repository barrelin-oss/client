...
death_dialog::death_dialog()
    : dialog(dialog_type::death)
{
    set_title("");
...
void death_dialog::set_death_info(std::string_view killer_name, bool is_pvp,
                                   int32_t xp_lost)
{
...
void death_dialog::render(renderer& rend)
{
    ...
    rend.draw_rect(actual_x_, actual_y_, dialog_width, dialog_height,
                   sf::Color(20, 10, 10, 220), true);

    // Border
    rend.draw_rect(actual_x_, actual_y_, dialog_width, dialog_height,
                   sf::Color(120, 40, 40), false);
    rend.draw_rect(actual_x_ + 1, actual_y_ + 1, dialog_width - 2, dialog_height - 2,
                   sf::Color(80, 30, 30), false);
    ...
    rend.draw_text_outlined(title, actual_x_ + (dialog_width - title_w) / 2, actual_y_ + 12,
                            sf::Color(255, 80, 80), outline, 14, 1.0f);
    ...
    if (!killer_name_.empty())
    {
        ...
        rend.draw_text_outlined(line, actual_x_ + (dialog_width - line_w) / 2, info_y,
                                sf::Color(220, 200, 180), outline, 12, 1.0f);
        info_y += 18;
    }
    ...
    if (resurrect_available_)
    {
        ...
        // Restart button
        {
            ...
            rend.draw_rect(bx, buttons_y, button_width, button_height,
                           sf::Color(120, 60, 60), false);

            const char* text = "Restart";
            int32_t tw = static_cast<int32_t>(std::strlen(text)) * 7;
            rend.draw_text_outlined(text, bx + (button_width - tw) / 2,
                                    buttons_y + 5, sf::Color::White, outline, 12, 1.0f);
        }
        ...
        // Resurrect button
        {
            ...
            rend.draw_rect(bx, buttons_y, button_width, button_height, bg, true);
            rend.draw_rect(bx, buttons_y, button_width, button_height,
                           sf::Color(60, 120, 60), false);

            const char* text = "Resurrect";
            int32_t tw = static_cast<int32_t>(std::strlen(text)) * 7;
            rend.draw_text_outlined(text, bx + (button_width - tw) / 2,
                                    buttons_y + 5, sf::Color(150, 255, 150), outline, 12, 1.0f);
        }
    }
    else
    {
...
{
    if (!is_open() || btn != sf::Mouse::Button::Left) return false;
