...
void dialog_callbacks::setup_callbacks()
{
    ...
    {
        create_dlg->set_on_create(
            [this](const character_create_dialog::character_data& data)
            {
    ...
    {
        spell_dlg->set_on_spell_click(
            [&magic, &ui](uint16_t spell_id)
            {
    ...
    {
        skill_dlg->set_on_skill_click([](uint16_t skill_id) { spdlog::debug("Skill {} clicked", skill_id); });
    }
    ...
    if (auto* guild_dlg = dynamic_cast<guild_dialog*>(ui.get_dialog(dialog_type::guild)))
    {
        ...
        guild_dlg->set_on_invite([&ws](std::string_view target) { ws.request_guild_invite(target); });

        guild_dlg->set_on_kick([&ws](std::string_view target) { ws.request_guild_kick(target); });

        guild_dlg->set_on_promote([&ws](std::string_view target) { ws.request_guild_promote(target); });

        guild_dlg->set_on_demote([&ws](std::string_view target) { ws.request_guild_demote(target); });

        guild_dlg->set_on_leave([&ws]() { ws.request_guild_leave(); });

        guild_dlg->set_on_disband([&ws]() { ws.request_guild_disband(); });

        guild_dlg->set_on_set_motd([&ws](std::string_view motd) { ws.request_guild_set_motd(motd); });

        guild_dlg->set_on_accept_invite([&ws]() { ws.request_guild_invite_respond(true); });

        guild_dlg->set_on_decline_invite([&ws]() { ws.request_guild_invite_respond(false); });
        ...
        guild.set_on_change(
            [this]()
            {
                ...
                if (g.has_pending_invite())
                {
                    ...
                    dlg->set_pending_invite(inv.guild_name, inv.guild_tag, inv.inviter_name, inv.time_remaining);
                }
                else
                {
    ...
    {
        trade_dlg->set_on_add_item([&network](int32_t slot) { network.request_trade_add_item(slot, slot); });

        trade_dlg->set_on_remove_item([&network](int32_t slot) { network.request_trade_remove_item(slot); });

        trade_dlg->set_on_confirm([&network]() { network.request_trade_confirm(); });

        trade_dlg->set_on_cancel([&network]() { network.request_trade_cancel(); });

        trade_dlg->set_on_set_gold([&network](uint32_t amount) { network.request_trade_set_gold(amount); });
    }

    // Craft dialog - item manufacturing
    if (auto* craft_dlg = dynamic_cast<craft_dialog*>(ui.get_dialog(dialog_type::manufacture)))
    {
        craft_dlg->set_on_craft([&network](int32_t recipe_index) { network.request_craft_item(recipe_index); });
    ...
    {
        repair_dlg->set_on_repair([&network](int32_t inventory_slot) { network.request_repair_item(inventory_slot); });

        repair_dlg->set_on_repair_all([&network]() { network.request_repair_all(); });
    }

    // Map dialog - click on location
    if (auto* map_dlg = dynamic_cast<map_dialog*>(ui.get_dialog(dialog_type::map)))
    {
        map_dlg->set_on_click_location([](int32_t x, int32_t y) { spdlog::debug("Map clicked at ({}, {})", x, y); });
    }
    ...
    if (auto* settings_dlg = dynamic_cast<settings_dialog*>(ui.get_dialog(dialog_type::options)))
    {
        ...
        // === Game tab callbacks ===
        settings_dlg->set_on_style_change(
            [this](ui_style style)
            {
        ...
        settings_dlg->set_on_show_damage_numbers_change(
            [](bool v)
            {
        ...
        settings_dlg->set_on_show_names_change(
            [](bool v)
            {
        ...
        settings_dlg->set_on_show_guild_names_change(
            [](bool v)
            {
        ...
        settings_dlg->set_on_show_hp_bars_change(
            [](bool v)
            {
        ...
        settings_dlg->set_on_camera_shake_change(
            [](bool v)
            {
        ...
        settings_dlg->set_on_show_weather_change(
            [this](bool v)
            {
        ...
        settings_dlg->set_on_show_tint_change(
            [this](bool v)
            {
        ...
        settings_dlg->set_on_type_to_chat_change(
            [this](bool enabled)
            {
        ...
        // === Video tab callbacks ===
        settings_dlg->set_on_resolution_change(
            [this](
        ...
        settings_dlg->set_on_framerate_change(
            [this](uint32_t fps)
            {
        ...
        settings_dlg->set_on_vsync_change(
            [this](bool vsync)
            {
        ...
        settings_dlg->set_on_remember_position_change(
            [](bool remember)
            {
        ...
        // === Audio tab callbacks ===
        settings_dlg->set_on_master_volume_change(
            [this](float volume)
            {
        ...
        settings_dlg->set_on_music_volume_change(
            [this](float volume)
            {
                if (auto* a = game_->get_audio())
                {
        ...
        settings_dlg->set_on_sound_volume_change(
            [this](float volume)
            {
                if (auto* a = game_->get_audio())
                {
        ...
        settings_dlg->set_on_music_enabled_change(
            [this](bool enabled)
            {
        ...
        settings_dlg->set_on_sfx_enabled_change(
            [this](bool enabled)
            {
        ...
        settings_dlg->set_on_show_timestamps_change(
            [](bool v)
            {
        ...
        settings_dlg->set_on_filter_profanity_change(
            [this](bool v)
            {
        ...
        settings_dlg->set_on_block_spam_change(
            [](bool v)
            {
        ...
        // === Debug tab callbacks ===
        settings_dlg->set_on_show_debug_stats_change(
            [](bool show)
            {
        ...
        settings_dlg->set_on_show_fps_change(
            [](bool show)
            {
        ...
        settings_dlg->set_on_show_entity_info_change(
            [](bool show)
            {
        ...
        settings_dlg->set_on_verbose_logging_change(
            [](bool verbose)
            {
