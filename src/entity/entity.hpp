...
public:
    ...
    // Optional components - check before use
    bool has_stats() const { return stats_.has_value(); }
    stats_component& stats() { return stats_.value(); }
    const stats_component& stats() const { return stats_.value(); }
    void add_stats() { stats_.emplace(); }

    bool has_combat() const { return combat_.has_value(); }
    combat_component& combat() { return combat_.value(); }
    const combat_component& combat() const { return combat_.value(); }
    void add_combat() { combat_.emplace(); }

    bool has_name() const { return name_.has_value(); }
    name_component& name() { return name_.value(); }
    const name_component& name() const { return name_.value(); }
    void add_name() { name_.emplace(); }

    bool has_movement() const { return movement_.has_value(); }
    movement_component& movement() { return movement_.value(); }
    const movement_component& movement() const { return movement_.value(); }
    void add_movement() { movement_.emplace(); }

    bool has_npc() const { return npc_.has_value(); }
    npc_component& npc() { return npc_.value(); }
    const npc_component& npc() const { return npc_.value(); }
    void add_npc() { npc_.emplace(); }

    bool has_monster() const { return monster_.has_value(); }
    monster_component& monster() { return monster_.value(); }
    const monster_component& monster() const { return monster_.value(); }
    void add_monster() { monster_.emplace(); }

    bool has_item() const { return item_.has_value(); }
    item_component& item() { return item_.value(); }
    const item_component& item() const { return item_.value(); }
    void add_item() { item_.emplace(); }

    bool has_effect() const { return effect_.has_value(); }
    effect_component& effect() { return effect_.value(); }
    const effect_component& effect() const { return effect_.value(); }
...
private:
    ...
    // Motion state
    object_action current_action_ = object_action::stop;
    int32_t target_x_ = 0;
    int32_t target_y_ = 0;
