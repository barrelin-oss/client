    ...
    uint32_t width, uint32_t height, bool fullscreen, bool borderless, int32_t monitor_x, int32_t monitor_y)
{
    ...
    if (window_.isOpen())
    {
        ...
        prev_center = sf::Vector2i{pos.x + static_cast<int>(width_) / 2, pos.y + static_cast<int>(height_) / 2};
        window_.close();
    }
...
{
    if (internal_width_ == w && internal_height_ == h)
        return;
...
void renderer::end_scene()
{
    ...
    else if (view_mode_ == view_mode::extended)
    {
        ...
        case fog_style::solid:
            draw_fog_solid(fair, dw, dh);
            break;
        case fog_style::tile_fog:
            draw_fog_tile(fair, dw, dh);
            break;
        case fog_style::vignette:
            draw_fog_vignette(fair, dw, dh);
            break;
        case fog_style::gradient:
            draw_fog_gradient(fair, dw, dh);
            break;
        }
...
{
    if (view_mode_ == view_mode::special)
        return width_;
    return internal_width_;
}
...
sf::IntRect renderer::fair_bounds() const
{
    if (view_mode_ == view_mode::extended)
    {
        ...
        return sf::IntRect({fair_x, fair_y},
...
bool renderer::is_in_fair_zone(int32_t display_x, int32_t display_y) const
{
    if (view_mode_ != view_mode::extended)
        return true;
    auto fair = fair_bounds();
    return display_x >= fair.position.x && display_x < fair.position.x + fair.size.x && display_y >= fair.position.y &&
           display_y < fair.position.y + fair.size.y;
}
...
void renderer::draw_fog_tile(const sf::IntRect& fair, int32_t dw, int32_t dh)
{
    ...
    sf::Color fog(0, 0, 0, 200);
    if (st > 0)
        draw_rect(0, 0, dw, st, fog);
    if (sb < dh)
        draw_rect(0, sb, dw, dh - sb, fog);
    if (sl > 0)
        draw_rect(0, st, sl, sb - st, fog);
    if (sr < dw)
        draw_rect(sr, st, dw - sr, sb - st, fog);
}
...
void renderer::draw_fog_gradient(const sf::IntRect& fair, int32_t dw, int32_t dh)
{
    ...
    if (igt > 0)
        draw_rect(0, 0, dw, igt, dark_c);
    if (igb < dh)
        draw_rect(0, igb, dw, dh - igb, dark_c);
    if (igl > 0)
        draw_rect(0, igt, igl, igb - igt, dark_c);
    if (igr < dw)
        draw_rect(igr, igt, dw - igr, igb - igt, dark_c);
}
...
void renderer::draw_targeting_boundary(const sf::IntRect& fair)
{
    ...
    sf::RectangleShape glow_rect({static_cast<float>(fair.size.x) + glow_thickness * 2.0f,
                                  static_cast<float>(fair.size.y) + glow_thickness * 2.0f});
    glow_rect.setPosition(
        {static_cast<float>(fair.position.x) - glow_thickness, static_cast<float>(fair.position.y) - glow_thickness});
    ...
    sf::RectangleShape border_rect({static_cast<float>(fair.size.x), static_cast<float>(fair.size.y)});
    border_rect.setPosition({static_cast<float>(fair.position.x), static_cast<float>(fair.position.y)});
...
void renderer::draw_sprite_additive_alpha(const sprite& spr, int32_t x, int32_t y, uint32_t frame, float alpha)
{
    ...
    static const sf::BlendMode additive_blend(sf::BlendMode::Factor::One,
...
void renderer::draw_sprite_additive_tinted(const sprite& spr,
                                           int32_t x,
                                           int32_t y,
                                           uint32_t frame,
                                           float alpha,
                                           ...
                                           int16_t b_offset)
{
    ...
    additive_shader_.setUniform("texture", sf::Shader::CurrentTexture);
    additive_shader_.setUniform("alpha", alpha * additive_intensity_);
    additive_shader_.setUniform("rgb_offset",
                                sf::Glsl::Vec3(static_cast<float>(r_offset) / 255.0f,
...
void renderer::draw_sprite_tinted(
...
void renderer::draw_text_outlined(std::string_view text,
                                  int32_t x,
                                  int32_t y,
                                  sf::Color color,
...
{
    std::array<sf::Vertex, 2> line = {sf::Vertex{{static_cast<float>(x1), static_cast<float>(y1)}, color},
