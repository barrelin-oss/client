...
shop_dialog::shop_dialog()
    : dialog(dialog_type::shop) {
    set_title("Shop");
    set_bounds({
        static_cast<int32_t>(screen_width) / 2 - 175,
        static_cast<int32_t>(screen_height) / 2 - 180,
        350,
        360
    });
    set_draggable(true);
...
bool shop_dialog::handle_mouse_down(int32_t x, int32_t y, sf::Mouse::Button btn) {
    ...
    if (selected_index_.has_value()) {
        ...
        if (minus_btn.contains(x, y)) {
            if (buy_quantity_ > 1) buy_quantity_--;
            return true;
        }
...
shop_sell_dialog::shop_sell_dialog()
    : dialog(dialog_type::shop_sell) {
