...
{
    return sf::Color(static_cast<uint8_t>(c.value("r", 255)),
...
{
    return sf::Color(static_cast<uint8_t>(c["r"] ? c["r"].as<int>(255) : 255),
...
element_type parse_element_type(const std::string& type_str)
{
    ...
    if (type_str == "text_input")
        return element_type::text_input;
    if (type_str == "image")
        return element_type::image;
    if (type_str == "progress_bar")
        return element_type::progress_bar;
    if (type_str == "checkbox")
        return element_type::checkbox;
    if (type_str == "slider")
        return element_type::slider;
    if (type_str == "list_box")
        return element_type::list_box;
    if (type_str == "grid")
        return element_type::grid;
    if (type_str == "sprite")
        return element_type::sprite;
    if (type_str == "panel")
        return element_type::panel;
    ...
    if (type_str == "hp_bar")
        return element_type::hp_bar;
    if (type_str == "mp_bar")
        return element_type::mp_bar;
    if (type_str == "sp_bar")
        return element_type::sp_bar;
    if (type_str == "exp_bar")
        return element_type::exp_bar;
    if (type_str == "location_text")
        return element_type::location_text;
    if (type_str == "combat_indicator")
        return element_type::combat_indicator;
    if (type_str == "super_attack")
        return element_type::super_attack;
...
{
    if (action_str == "open_dialog")
        return element_action::open_dialog;
    if (action_str == "close_dialog")
        return element_action::close_dialog;
    if (action_str == "close_self")
        return element_action::close_self;
    if (action_str == "toggle_dialog")
        return element_action::toggle_dialog;
...
dialog_definition parse_dialog_yaml(const YAML::Node& dlg_yaml)
{
    ...
    def.right_click_closeable =
        dlg_yaml["right_click_closeable"] ? dlg_yaml["right_click_closeable"].as<bool>(true) : true;
...
managed_dialog* dialog_manager::create_dialog(std::string_view definition_id)
{
    ...
    dlg->set_action_handler(
        [this](element_action action, std::string_view target)
        {
            switch (action)
            {
    ...
    else
    {
        ...
        auto insert_pos = std::find_if(
            dialog_order_.begin(), dialog_order_.end(), [](managed_dialog* d) { return d->always_on_top(); });
...
void dialog_manager::destroy_dialog(std::string_view definition_id)
{
    ...
    {
        // Remove from order
        dialog_order_.erase(std::remove(dialog_order_.begin(), dialog_order_.end(), it->second.get()),
                            dialog_order_.end());
...
yaml_help_dialog* dialog_manager::create_help_dialog()
{
    ...
    for (const auto& yaml_path : yaml_paths_)
    {
        try
        {
            ...
            if (categories.empty())
            {
                categories = {{"All", -1}, {"Basics", 0}, {"Combat", 1}, {"Magic", 2}, {"Items", 3}, {"Keys", 5}};
            }
