...
character_select_dialog::character_select_dialog() : dialog(dialog_type::character_select)
{
    set_title("Select Character");
    set_bounds({static_cast<int32_t>(screen_width) / 2 - 350, static_cast<int32_t>(screen_height) / 2 - 170, 700, 340});
    set_closeable(true);
    set_modal(true);
...
void character_select_dialog::update(float delta_time, const input& inp)
{
    ...
    if (inp.is_mouse_pressed(sf::Mouse::Button::Left))
    {
        ...
        for (int32_t i = 0; i < max_characters; ++i)
        {
            ...
            if (mx >= slot_x && mx < slot_x + slot_width && my >= slot_y && my < slot_y + slot_height)
            {
...
void character_select_dialog::render(renderer& rend)
{
    ...
    if (!status_message_.empty())
    {
        ...
        rend.draw_text(status_message_, bounds_.x + 20, bounds_.y + 260, status_color);
    }
}
...
void character_select_dialog::render_character_slot(renderer& rend, int32_t slot, int32_t x, int32_t y)
{
    ...
    if (has_character)
    {
        ...
        rend.draw_text(gender_str + "/" + class_str, x + 60, y + 40, sf::Color(120, 120, 140));
        ...
        // Stats
        std::string stats1 = "STR:" + std::to_string(ch.strength) + " VIT:" + std::to_string(ch.vitality) +
