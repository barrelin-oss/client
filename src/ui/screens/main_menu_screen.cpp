...
void main_menu_screen::on_enter()
{
    ...
    mouse_interface_.add_rect(btn2_x_,
                              btn2_y_,
                              btn2_x_ + btn_width_,
                              btn2_y_ + btn_height_); // Button 2: Create Account
...
bool main_menu_screen::update(float delta_time, const input& inp)
{
    ...
    if (adj_x >= btn1_x_ && adj_x <= btn1_x_ + btn_width_ && adj_y >= btn1_y_ && adj_y <= btn1_y_ + btn_height_)
        current_focus_ = 1;
    if (adj_x >= btn2_x_ && adj_x <= btn2_x_ + btn_width_ && adj_y >= btn2_y_ && adj_y <= btn2_y_ + btn_height_)
        current_focus_ = 2;
    if (adj_x >= btn3_x_ && adj_x <= btn3_x_ + btn_width_ && adj_y >= btn3_y_ && adj_y <= btn3_y_ + btn_height_)
        current_focus_ = 3;
...
void main_menu_screen::draw(renderer& rend,
...
void main_menu_screen::register_debug_adapters()
{
    ...
    debug_adapters_.push_back(
        std::make_unique<debug::screen_point_adapter>("main_menu.btn_quit", btn3_x_, btn3_y_, btn_width_, btn_height_));
