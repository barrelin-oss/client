...
login_dialog::login_dialog() : dialog(dialog_type::login)
{
    set_title("Helbreath Login");
    set_bounds({static_cast<int32_t>(screen_width) / 2 - 180, static_cast<int32_t>(screen_height) / 2 - 140, 360, 280});
    set_closeable(false);
    set_draggable(false);
    set_modal(true);
...
void login_dialog::render(renderer& rend)
{
    ...
    if (!status_message_.empty())
    {
        ...
        rend.draw_text(status_message_, bounds_.x + 20, bounds_.y + 250, status_color);
    }
}
...
{
    if (!account_input_ || !password_input_)
        return;
