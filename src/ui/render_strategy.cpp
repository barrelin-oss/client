...
void modern_render_strategy::render_background(renderer& rend, const dialog_definition& def, const ui_bounds& bounds)
{
    ...
    if (def.has_border)
    {
        ...
        // Inner highlight
        rend.draw_line(
            bounds.x + 1, bounds.y + 1, bounds.x + bounds.width - 2, bounds.y + 1, sf::Color(100, 100, 120, 100));
        rend.draw_line(
            bounds.x + 1, bounds.y + 1, bounds.x + 1, bounds.y + bounds.height - 2, sf::Color(100, 100, 120, 100));
    }
}
                                              ...
                                              bool close_hovered)
{
    ...
    rend.draw_line(
        bounds.x, bounds.y + title_height, bounds.x + bounds.width, bounds.y + title_height, sf::Color(80, 80, 100));
    ...
    if (def.closeable)
    {
        ...
        sf::Color close_bg = close_hovered ? sf::Color(180, 60, 60) : sf::Color(120, 60, 60);

        rend.draw_rect(close_x, close_y, close_size, close_size, close_bg, true);
        rend.draw_rect(close_x, close_y, close_size, close_size, sf::Color(150, 100, 100), false);
...
void modern_render_strategy::render_button(renderer& rend,
                                           const element_def& elem,
                                           const element_state& state,
                                           const ui_bounds& bounds)
{
    ...
    if (state.pressed)
    {
        rend.draw_line(bounds.x, bounds.y, bounds.x + bounds.width - 1, bounds.y, sf::Color(30, 30, 40));
        rend.draw_line(bounds.x, bounds.y, bounds.x, bounds.y + bounds.height - 1, sf::Color(30, 30, 40));
        rend.draw_line(bounds.x + bounds.width - 1,
                       bounds.y,
                       bounds.x + bounds.width - 1,
                       bounds.y + bounds.height - 1,
                       sf::Color(100, 100, 120));
        rend.draw_line(bounds.x,
                       bounds.y + bounds.height - 1,
                       bounds.x + bounds.width - 1,
                       bounds.y + bounds.height - 1,
                       sf::Color(100, 100, 120));
    ...
    {
        rend.draw_line(bounds.x, bounds.y, bounds.x + bounds.width - 1, bounds.y, sf::Color(100, 100, 120));
        rend.draw_line(bounds.x, bounds.y, bounds.x, bounds.y + bounds.height - 1, sf::Color(100, 100, 120));
        rend.draw_line(bounds.x + bounds.width - 1,
                       bounds.y,
                       bounds.x + bounds.width - 1,
                       bounds.y + bounds.height - 1,
    ...
    if (!text.empty())
    {
        ...
        rend.draw_text(text, text_x, text_y, text_color, elem.font_size.value_or(11));
    }
}

void modern_render_strategy::render_label(renderer& rend,
                                          const element_def& elem,
                                          const element_state& state,
                                          const ui_bounds& bounds)
{
    ...
    rend.draw_text(text, bounds.x, bounds.y, color, elem.font_size.value_or(11));
}

void modern_render_strategy::render_text_input(renderer& rend,
                                               const element_def& elem,
                                               const element_state& state,
                                               const ui_bounds& bounds)
{
    sf::Color bg = elem.background_color.value_or(sf::Color(25, 25, 35));
    sf::Color border = state.focused ? sf::Color(100, 140, 200) : elem.border_color.value_or(sf::Color(60, 60, 80));
    ...
    if (state.focused)
    {
        ...
        rend.draw_rect(cursor_x, bounds.y + 4, 1, bounds.height - 8, sf::Color::White, true);
    }
}

void modern_render_strategy::render_progress_bar(renderer& rend,
                                                 const element_def& elem,
                                                 const element_state& state,
                                                 const ui_bounds& bounds)
{
    ...
    if (fill_width > 0)
    {
        ...
        // Highlight
        sf::Color highlight(static_cast<uint8_t>(std::min(255, fill.r + 40)),
...
void modern_render_strategy::render_checkbox(renderer& rend,
                                             const element_def& elem,
                                             const element_state& state,
                                             const ui_bounds& bounds)
{
    ...
    sf::Color border = state.hovered ? sf::Color(100, 140, 200) : elem.border_color.value_or(sf::Color(70, 70, 90));
...
void modern_render_strategy::render_slider(renderer& rend,
                                           const element_def& elem,
                                           const element_state& state,
                                           const ui_bounds& bounds)
{
    ...
    sf::Color thumb_color = state.pressed ? sf::Color(140, 180, 220) : sf::Color(100, 150, 200);
    rend.draw_rect(thumb_x, thumb_y, 12, 16, thumb_color, true);
    rend.draw_rect(thumb_x, thumb_y, 12, 16, sf::Color(150, 180, 220), false);
}

void modern_render_strategy::render_panel(renderer& rend,
                                          const element_def& elem,
...
void modern_render_strategy::render_separator(renderer& rend, const element_def& elem, const ui_bounds& bounds)
{
...
void modern_render_strategy::render_grid(renderer& rend,
                                         const element_def& elem,
                                         const element_state& state,
                                         const ui_bounds& bounds)
{
...
void classic_render_strategy::render_background(renderer& rend, const dialog_definition& def, const ui_bounds& bounds)
{
...
void classic_render_strategy::render_sprite_button(renderer& rend,
                                                   const element_def& elem,
                                                   const element_state& state,
                                                   const ui_bounds& bounds)
{
...
void classic_render_strategy::render_sprite_image(renderer& rend, const element_def& elem, const ui_bounds& bounds)
{
    if (!sprites_)
        return;
