...
bool login_screen::update(float delta_time, const input& inp)
{
    ...
    if (adj_x >= 80 && adj_x <= 163 && adj_y >= 280 && adj_y <= 302)
        current_focus_ = 3;
    if (adj_x >= 258 && adj_x <= 327 && adj_y >= 280 && adj_y <= 302)
        current_focus_ = 4;
...
void login_screen::register_debug_adapters()
{
    ...
    debug_adapters_.push_back(std::make_unique<debug::screen_point_adapter>(
        "login_screen.account_box", account_box_x_, account_box_y_, input_box_width_, input_box_height_));

    // Password input box adapter
    debug_adapters_.push_back(std::make_unique<debug::screen_point_adapter>(
        "login_screen.password_box", password_box_x_, password_box_y_, input_box_width_, input_box_height_));
