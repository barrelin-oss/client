...
craft_dialog::craft_dialog()
    : dialog(dialog_type::manufacture) {
...
void craft_dialog::render_recipe_list(renderer& rend, int32_t x, int32_t y) {
    ...
    if (recipes_.size() > static_cast<size_t>(visible_recipes)) {
        ...
        if (scroll_offset_ + visible_recipes < static_cast<int32_t>(recipes_.size())) {
            rend.draw_text("v", x + recipe_width + 5, y + (visible_recipes - 1) * recipe_height, sf::Color(150, 150, 150));
        }
    }

    for (int32_t i = 0; i < visible_recipes; ++i) {
        int32_t recipe_idx = scroll_offset_ + i;
        if (recipe_idx >= static_cast<int32_t>(recipes_.size())) break;
    ...
    for (int32_t i = 0; i < max_ingredients; ++i) {
        ...
        if (ingredients_[i]) {
            ...
            if (ingredients_[i]->amount > 1) {
                rend.draw_text(std::to_string(ingredients_[i]->amount),
                               slot_x + 2, slot_y + slot_size - 14, sf::Color::White, 10);
            }
        }
    }
}
...
void craft_dialog::render_result_preview(renderer& rend, int32_t x, int32_t y) {
    if (selected_recipe_ < 0 || selected_recipe_ >= static_cast<int32_t>(recipes_.size())) return;
