...
void party_dialog::render_member_row(renderer& rend, const party_member_info& member, int32_t y, bool hovered)
{
    int32_t x = bounds_.x + 10;
...
{
    members_.erase(std::remove_if(members_.begin(),
                                  members_.end(),
...
guild_dialog::guild_dialog() : dialog(dialog_type::guild)
{
    set_title("Guild");
    set_bounds({static_cast<int32_t>(screen_width) / 2 - 175, static_cast<int32_t>(screen_height) / 2 - 175, 350, 350});
    set_draggable(true);
    set_closeable(true);
    set_visible(false);
}
...
void guild_dialog::render_invite_banner(renderer& rend, int32_t x, int32_t y)
{
    ...
    // Timer
    rend.draw_text(std::format("{:.0f}s", invite_time_remaining_), x + 260, btn_y + 2, sf::Color(180, 180, 180), 10);
}
...
void guild_dialog::render_no_guild(renderer& rend, int32_t x, int32_t y)
{
    ...
    rend.draw_rect(x + 50, y, 220, 20, name_active ? sf::Color(60, 60, 90) : sf::Color(40, 40, 55), true);
    rend.draw_rect(x + 50, y, 220, 20, name_active ? sf::Color(120, 120, 180) : sf::Color(70, 70, 90), false);
    ...
    rend.draw_rect(x + 50, y, 80, 20, tag_active ? sf::Color(60, 60, 90) : sf::Color(40, 40, 55), true);
    rend.draw_rect(x + 50, y, 80, 20, tag_active ? sf::Color(120, 120, 180) : sf::Color(70, 70, 90), false);
...
void guild_dialog::render_guild_view(renderer& rend, int32_t x, int32_t y)
{
    ...
    rend.draw_rect(x + 50, bottom_y, 150, 20, invite_active ? sf::Color(60, 60, 90) : sf::Color(40, 40, 55), true);
    rend.draw_rect(x + 50, bottom_y, 150, 20, invite_active ? sf::Color(120, 120, 180) : sf::Color(70, 70, 90), false);
...
{
    if (!visible_ || active_field_ == text_field::none)
        return false;
    ...
    if (key == sf::Keyboard::Key::Backspace)
    {
        ...
        case text_field::create_name:
            target = &create_name_input_;
            break;
        case text_field::create_tag:
            target = &create_tag_input_;
            break;
        case text_field::invite_target:
            target = &invite_input_;
            break;
...
void guild_dialog::set_pending_invite(std::string_view guild_name,
