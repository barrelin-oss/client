...
connection_dialog::connection_dialog() : dialog(dialog_type::connection)
{
    set_title("");
...
void connection_dialog::render(renderer& rend)
{
    ...
    rend.draw_rect(
        0, 0, static_cast<int32_t>(window_width_), static_cast<int32_t>(window_height_), sf::Color(0, 0, 0, 128), true);

    // Draw dialog box background
    rend.draw_rect(actual_x_, actual_y_, dialog_width, dialog_height, sf::Color(32, 32, 48, 240), true);

    // Draw border
    rend.draw_rect(actual_x_, actual_y_, dialog_width, dialog_height, sf::Color(80, 80, 120), false);
    rend.draw_rect(actual_x_ + 1, actual_y_ + 1, dialog_width - 2, dialog_height - 2, sf::Color(60, 60, 90), false);
    ...
    if (error_mode_)
    {
        ...
        rend.draw_text_outlined(error_message_, text_x, text_y, sf::Color(255, 200, 200), outline_color);
        ...
        rend.draw_text_outlined(btn_text,
                                btn_text_x,
                                btn_text_y,
    ...
    else
    {
        ...
        rend.draw_text_outlined(main_msg, main_x, main_y, sf::Color(255, 255, 200), outline_color);
...
{
    if (!visible_ || btn != sf::Mouse::Button::Left)
        return false;
...
bool connection_dialog::is_point_in_cancel_button(int32_t x, int32_t y) const
{
    int32_t btn_x = actual_x_ + button_rel_x;
    int32_t btn_y = actual_y_ + button_rel_y;
    return x >= btn_x && x < btn_x + button_width && y >= btn_y && y < btn_y + button_height;
}
